...
public abstract class RequestProcessor implements Runnable, Cacheable {
    ...
    /**
     * Tests whether it is forbidden or not to handle this request.
     * 
     * @return <code>true</code> if it is forbidden to handle the request;
     * otherwise <code>false</code>.
     *
     * @since 1.3
     */
    public final boolean isForbidden() {
        ...
        if (getRequest() != null && getAttribute("io.pictura.servlet.IP_ADDRESS_MATCH") != null) {
            final String[] ipAddrMatch = (String[]) getAttribute("io.pictura.servlet.IP_ADDRESS_MATCH");
            boolean forbidden = true;
            for (String ipAddrRange : ipAddrMatch) {
                if (new IpAddressMatcher(ipAddrRange.trim()).matches(getRequest().getRemoteAddr())) {
                    forbidden = false;
                    break;
                }
            }
     ...
     *
     * @param resLocators The defined resource locators for this processor or
     * <code>null</code> if there are no resource locators are set.
     *
     * @throws IllegalStateException if the method was called after the request
     * processor was comitted.
     */
