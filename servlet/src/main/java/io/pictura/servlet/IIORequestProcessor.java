...
/**
 * Extends the {@link RequestProcessor} with basic ImageIO support depending on
...
public abstract class IIORequestProcessor extends RequestProcessor {
     ...
     * 
     * @deprecated As of version 1.1 this method is not longer supported by
     * <code>IIORequestProcessor</code>'s. Use
     * {@link #doWrite(byte[], int, int, javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse)}
     * instead of this, if you need to intercept the encoded image data.
     */
    @Deprecated
    @Override    
    protected long doWrite(byte[] data, HttpServletRequest req,
        ...
        return super.doWrite(data, req, resp);
    }
     ...
     * <p>
     * If there is an {@link ImageInterceptor} is defined in the request scope
     * (attribute <code>io.pictura.servlet.IIO_IMAGE_INTERCEPTOR</code>), the
     * interceptor is called for each image frame, before the result will be
    ...
    private void doWriteImage0(byte[] data, int off, int len, IIOWriteParam param,
            HttpServletRequest req, HttpServletResponse resp) throws
            ServletException, IOException {
